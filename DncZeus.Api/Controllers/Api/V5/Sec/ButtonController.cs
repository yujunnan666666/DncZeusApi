...
using DncZeus.Api.ViewModels.Sec.Button;
...
namespace DncZeus.Api.Controllers.Api.Sec
{
    ...
    public class ButtonController : ControllerBase
    {
        ...
        public ButtonController(DncZeusDbContext dbContext, IMapper mapper)
        {
        ...
        [HttpPost]
        public IActionResult List(ButtonRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Secbutton.AsQueryable();
                ...
                if (!string.IsNullOrEmpty(payload.Kw))
                {
                    ...
                    x.butname.Contains(payload.Kw.Trim())
                    )
                    );
                }
                query=query.OrderBy("butorder", false);
        ...
        public IActionResult Create(ButtonCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                var entity = _mapper.Map<ButtonCreateViewModel, Secbutton>(model);
                entity.Guid = new Guid();
                
                _dbContext.Secbutton.Add(entity);
        ...
        public IActionResult Detail(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Secbutton.FirstOrDefault(x => x.Guid == guid);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<Secbutton, ButtonCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(ButtonCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
           /* if (model.Code.Trim().Length <= 0)
            ...
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Secbutton.FirstOrDefault(x => x.Guid == model.Guid);
                entity.butno = model.butno;
        ...
        public ResponseModel Delete(string ids)
        {
            using (_dbContext)
            {
                ...
                try {
                    ...
                    var sql = string.Format("DELETE FROM Secbutton WHERE Guid IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateIsValid(string enabled, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new SqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE Secbutton SET enabled=@enabled WHERE Guid IN ({0})", parameterNames);
