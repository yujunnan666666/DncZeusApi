...
using DncZeus.Api.RequestPayload.Sec.Sys;
using DncZeus.Api.ViewModels.Sec.Sys;
...
namespace DncZeus.Api.Controllers.Api.Sec
{
    ...
    public class SysController : ControllerBase
    {
        ...
        public SysController(DncZeusDbContext dbContext, IMapper mapper)
        {
        ...
        [HttpPost]
        public IActionResult List(SysRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Secsys.AsQueryable();
                ...
                if (!string.IsNullOrEmpty(payload.Kw))
                {
                    query = query.Where(x =>
                    (
                    x.sysno.Contains(payload.Kw.Trim()) ||
                    x.sysname.Contains(payload.Kw.Trim())
                    )
                    );
                }
                query=query.OrderBy("sysorder", false);
        ...
        public IActionResult Create(SysCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                var entity = _mapper.Map<SysCreateViewModel, Secsys>(model);
                if (_dbContext.Secsys.Count(x => x.sysno == model.sysno) > 0)
                {
                    response.SetFailed("系统编号已存在");
                    return Ok(response);
                }
                entity.Guid = new Guid();
                
                _dbContext.Secsys.Add(entity);
        ...
        public IActionResult Detail(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Secsys.FirstOrDefault(x => x.Guid == guid);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<Secsys, SysCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(SysCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
           /* if (model.Code.Trim().Length <= 0)
            ...
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Secsys.FirstOrDefault(x => x.Guid == model.Guid);
                entity.sysno = model.sysno;
                entity.sysname = model.sysname;
                
                entity.sysorder = model.sysorder;
        ...
        public ResponseModel Delete(string ids)
        {
            using (_dbContext)
            {
                ...
                try {
                    ...
                    var sql = string.Format("DELETE FROM Secsys WHERE Guid IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateIsValid(string enabled, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new SqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE Secsys SET enabled=@enabled WHERE Guid IN ({0})", parameterNames);
