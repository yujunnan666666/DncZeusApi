...
using DncZeus.Api.RequestPayload.Pmc.Orderday;
using DncZeus.Api.ViewModels.Pmc.Orderday;
...
namespace DncZeus.Api.Controllers.Api.Pmc
{
    ...
    public class OrderdayController : ControllerBase
    {
        private readonly DncZeusDbContext _dbContext;
        private readonly IMapper _mapper;
        

        public OrderdayController(DncZeusDbContext dbContext, IMapper mapper)
        {
        ...
        [HttpPost]
        public IActionResult List(OrderdayRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.PmcOrderday.AsQueryable();
        ...
        public IActionResult Create(OrderdayCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                var entity = _mapper.Map<OrderdayCreateViewModel, PmcOrderday>(model);

                /*if (_dbContext.PmcOrderday.Count(x => x.Code == model.Code) > 0)
                {
                ...
                _dbContext.PmcOrderday.Add(entity);
        ...
        public IActionResult Detail(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.PmcOrderday.FirstOrDefault(x => x.Guid == guid);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<PmcOrderday, OrderdayCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(OrderdayCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
           /* if (model.Code.Trim().Length <= 0)
            ...
            using (_dbContext)
            {
                ...
                var entity = _dbContext.PmcOrderday.FirstOrDefault(x => x.Guid == model.Guid);
        ...
        public ResponseModel Delete(string ids)
        {
            using (_dbContext)
            {
                ...
                try {
                    ...
                    var sql = string.Format("DELETE FROM PmcOrderday WHERE Guid IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(Status status, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = "";
                    ...
                    {
                        sql = "UPDATE PmcOrderday SET status=2, cfmuser='{1}', cfmdate=getdate() WHERE  Guid IN ({0})";
                        sql = string.Format(sql, parameterNames, AuthContextService.CurrentUser.DisplayName);
                    }
                    else
                    {
                        sql = "UPDATE PmcOrderday SET status=3, cfmuser='{1}',cfmdate=getdate() WHERE  Guid IN ({0})";
                        sql = string.Format(sql, parameterNames, AuthContextService.CurrentUser.DisplayName);
                    }
        ...
        private ResponseModel UpdateIsValid(string enabled, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new SqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE PmcOrderday SET enabled=@enabled WHERE Guid IN ({0})", parameterNames);
