...
using DncZeus.Api.RequestPayload.Pmc.Jobuser;
using DncZeus.Api.ViewModels.Pmc.Jobuser;
...
namespace DncZeus.Api.Controllers.Api.Pmc
{
    ...
    public class JobuserController : ControllerBase
    {
        private readonly DncZeusDbContext _dbContext;
        private readonly IMapper _mapper;
        

        public JobuserController(DncZeusDbContext dbContext, IMapper mapper)
        {
        ...
        [HttpPost]
        public IActionResult List(JobuserRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                //var query = _dbContext.PmcJobuser.AsQueryable();
                ...
                var query = _dbContext.PmcJobuser.AsQueryable().GroupJoin(_dbContext.MisCode, a => a.jobId, b => b.ID, (a, b) => new {
                    PmcJobuser=a,
                    MisCode = b
                }).SelectMany(a => a.MisCode, (a, b) => new {
                    a.PmcJobuser,
                    b.cdesc
                }).GroupJoin(_dbContext.Secuser, a => a.PmcJobuser.userGuid, b => b.Guid, (a, b) => new {
                    a.PmcJobuser,
                    a.cdesc,
                    Secuser = b
                }).SelectMany(a => a.Secuser, (a, b) => new {
                    a.PmcJobuser.credate,
                    a.PmcJobuser.creuser,
                    a.PmcJobuser.Guid,
                    a.PmcJobuser.isvalid,
                    a.PmcJobuser.jobId, 
                    a.PmcJobuser.userGuid,
                    a.PmcJobuser.moddate,
                    a.PmcJobuser.moduser,
                    a.cdesc, 
                    b.username,
                    b.userno
                });
        ...
        public IActionResult Create(JobuserCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                var entity = _mapper.Map<JobuserCreateViewModel, PmcJobuser>(model);

                /*if (_dbContext.PmcJobuser.Count(x => x.Code == model.Code) > 0)
                {
                ...
                _dbContext.PmcJobuser.Add(entity);
        ...
        public IActionResult Detail(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.PmcJobuser.FirstOrDefault(x => x.Guid == guid);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<PmcJobuser, JobuserCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(JobuserCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
           /* if (model.Code.Trim().Length <= 0)
            ...
            using (_dbContext)
            {
                ...
                var entity = _dbContext.PmcJobuser.FirstOrDefault(x => x.Guid == model.Guid);
        ...
        public ResponseModel Delete(string ids)
        {
            using (_dbContext)
            {
                ...
                try {
                    ...
                    var sql = string.Format("DELETE FROM PmcJobuser WHERE Guid IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(Status status, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = "";
                    ...
                    {
                        sql = "UPDATE PmcJobuser SET status=2, cfmuser='{1}', cfmdate=getdate() WHERE  Guid IN ({0})";
                        sql = string.Format(sql, parameterNames, AuthContextService.CurrentUser.DisplayName);
                    }
                    else
                    {
                        sql = "UPDATE PmcJobuser SET status=3, cfmuser='{1}',cfmdate=getdate() WHERE  Guid IN ({0})";
                        sql = string.Format(sql, parameterNames, AuthContextService.CurrentUser.DisplayName);
                    }
        ...
        private ResponseModel UpdateIsValid(string enabled, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new SqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE PmcJobuser SET enabled=@enabled WHERE Guid IN ({0})", parameterNames);
